...
SphericalHarmonics::evaluateOrthonormal()
{
  ...
  switch (_orders[0])
  {
    ...
    save(24, std::sqrt(315 / (256 * M_PI)) * (x*x*(x*x - 3.0*y*y) - y*y*(3.0*x*x - y*y))/(rho4));
    save(23, std::sqrt(315 / (32 * M_PI)) * (x*x-3*y*y)*x*z/(rho4));
    save(22, std::sqrt(45 / (64 * M_PI)) * ((x*x-y*y)*(7*z*z-rho2))/(rho4));
    save(21, std::sqrt(45 / (32 * M_PI)) * (x*z*(7*z*z - 3*rho2))/(rho4));
    save(20, std::sqrt(9 / (256 * M_PI)) * (35*z*z*z*z - 30*z*z*rho2 + 3*rho4)/(rho4));
    save(19, std::sqrt(45 / (32 * M_PI)) * (y*z*(3*x*x - y*y))/(rho4));
    save(18, std::sqrt(45 / (16 * M_PI)) * (x*y*(7*z*z - 3*rho2))/(rho4));
    save(17, std::sqrt(315 / (32 * M_PI)) * (3*x*x - y*y)*(y*z)/(rho4));
    save(16, std::sqrt(315 / (16 * M_PI)) * (x*y*(x*x - y*y))/(rho4));
    libmesh_fallthrough();

    case 3:
    save(15, std::sqrt(35 / (32 * M_PI)) * (x*(x*x - 3.0*y*y))/(rho2*rho));
    save(14, std::sqrt(105 / (16 * M_PI)) * (z*(x*x - y*y))/(rho2*rho));
    save(13, std::sqrt(21 / (32 * M_PI)) * (x*(4*z*z - x*x - y*y))/(rho2*rho));
    save(12, std::sqrt(7 / (16 * M_PI)) * z*(2*z*z - 3*x*x - 3*y*y)/(rho2*rho));
    save(11, std::sqrt(21 / (32 * M_PI)) * (y*(4*z*z - x*x - y*y))/(rho2*rho));
    save(10, std::sqrt(105 / (4 * M_PI)) * (x*y*z)/(rho2*rho));
    save(9, std::sqrt(35 / (32 * M_PI)) * ((3 * x * x - y * y) * y) / (rho2 * rho));
    libmesh_fallthrough();

    case 2:
    save(8, std::sqrt(15 / (16 * M_PI)) * (x * x - y * y) / (rho2));
    save(7, std::sqrt(15/ (4 * M_PI)) * (z * x) / (rho2));
    save(6, std::sqrt(5 / (16*M_PI)) * (-x*x - y*y + 2.0*z*z)/(rho2));
    save(5, std::sqrt(15 / (4*M_PI)) * (y * z) / (rho2));
    save(4, std::sqrt(15 / (4*M_PI)) * (x * y) / (rho2));
    libmesh_fallthrough();

    case 1:
    save(3, std::sqrt(3 / (4 * M_PI)) * x / rho);
    save(2, std::sqrt(3 / (4 * M_PI)) * z / rho);
    save(1, std::sqrt(3 / (4 * M_PI)) * y / rho);
    libmesh_fallthrough();

    case 0:
    save(0, 1 / std::sqrt(4 * M_PI));
  }
}
...
//  static const std::vector<Real> getStandardizedFunctionLimits = {0.0, 1.0, 0, M_PI, 0, 2*M_PI};//0, M_PI, 0, 2*M_PI};
  static const std::vector<Real> getStandardizedFunctionLimits = {0, 1, -1, 1, -1, 1, -1, 1};
